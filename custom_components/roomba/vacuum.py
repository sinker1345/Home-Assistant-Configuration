...
CONF_MAP = 'map'
CONF_MAP_DIR = 'map_dir'
CONF_ICON_DIR = 'icon_dir'
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_MAP, default=DEFAULT_MAP): cv.boolean,
    vol.Optional(CONF_MAP_DIR, default=DEFAULT_MAP_DIR): cv.string,
    vol.Optional(CONF_ICON_DIR, default=DEFAULT_ICON_DIR): cv.string,
...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    map = config.get(CONF_MAP)
    map_dir = config.get(CONF_MAP_DIR)
    icon_dir = config.get(CONF_ICON_DIR)

    # If path is relative, we assume relative to HASS config dir
    if not os.path.isabs(map_dir):
        map_dir = hass.config.path(map_dir)

    if not os.path.isdir(map_dir):
        _LOGGER.error(
            "Map path %s does not exist.",
            map_dir)
        return False

    # If path is relative, we assume relative to HASS config dir
    if not os.path.isabs(icon_dir):
        icon_dir = hass.config.path(icon_dir)

    if not os.path.isdir(icon_dir):
        _LOGGER.error(
            "Icon path %s does not exist.",
            icon_dir)
        return False
